...
const { TROCIO_DB, TROCIO_BACKUP } = require('../config.js')
...
const backupCommand = `mongodump -d trocio -o ${TROCIO_BACKUP}/trocio-${new Date()
  .toISOString()
  .replace(/:|\./g, '-')}`
...
exec(backupCommand, (error, stdout, stderr) => {
  ...
  console.log(
    `Backup dumped on ${backupCommand.slice(backupCommand.indexOf('-o') + 3)}`
  )
  ...
  drop(async () => {
    ...
    try {
      let admin = await User({
        name: 'Admin',
        mail: 'jonas.voisard@gmail.com',
        password: '1234',
      }).save()
      let jonas = await User({
        name: 'Jonas',
      ...
      const trocVetement = await Troc({
        creator: admin._id,
        admin: [admin._id],
        name: `Echange de vêtements (évenement terminé)`,
        ...
        schedule: [
          {
            open: new Date(new Date().getTime() - 1000 * 60 * 60 * 24 * 2),
            close: new Date(new Date().getTime() - 1000 * 60 * 60 * 24 * 1),
          },
        ],
      }).save()
      ...
      for (i = 0; i < N; i++) {
        await Troc({
          ...
          location: {
            lat: 45 + Math.random() * 4,
            lng: 5 + Math.random() * 7,
          },
          description: `Déscription bidon`,
          schedule: [
            {
              open: new Date(new Date().getTime() - 1000 * 60 * 60 * 24 * i),
              close: new Date(
      ...
      const velos = [
        ...
        'Dérive-chaîne',
        'Maillons de secours',
        'Multi-outil de poche',
        'Pince ou outil type Leatherman',
