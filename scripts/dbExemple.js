...
const backupCommand = `mongodump -d trocio -o ${TROCIO_BACKUP}/trocio-${new Date().toISOString().replace(/:|\./g, '-')}`

exec(backupCommand, (error, stdout, stderr) => {
    if (error) return console.log('error', error.message)
    console.log('stdout', stdout)
    console.log('sdterr', stderr)
    console.log(`Backup dumped on ${backupCommand.slice(backupCommand.indexOf('-o') + 3)}`)
