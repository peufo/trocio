<script lang="ts">
    ...
    import { mdiLoading, mdiPrinter, mdiTagMultipleOutline } from '@mdi/js'
    ...
    let articlesProvidedTable: ArticleProvidedTable
    let printTagsPending = false
...
{:else if $queryResum.isSuccess && resum}
    <div
        in:fade|local
        class="d-flex flex-column pa-4 {klass}"
        style="gap: 1em;"
    >
        <div class="d-flex align-center" style="gap: 0.5em;">
            <a
                href={`/print-subscribe?subscribeId=${subscribeId}`}
                target="_blank"
            >
        ...
        >
            <ArticleProvidedTable
                bind:this={articlesProvidedTable}
                {modeAdmin}
                {subscribeId}
                on:openTarifDialog
            />
            ...
            <div slot="head">
                ...
                <Button
                    ...
                    disabled={!articlesProvidedTable || printTagsPending}
                    on:click={() => {
                        printTagsPending = true
                        articlesProvidedTable
                            .printTags()
                            .finally(() => (printTagsPending = false))
                    }}
                >
                    {#if printTagsPending}
                        <Icon path={mdiLoading} size="1.1em" spin />
                    {:else}
                        <Icon path={mdiTagMultipleOutline} size="1.1em" />
                    {/if}
                </Button>
            </div>
        </DetailCard>
