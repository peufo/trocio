<script lang="ts">
    ...
    export async function printTags() {
        const allArticles = await api<ParamsArticleAPI, ArticleLookup[]>(
            '/api/articles',
            {
                params: {
                    exact_providerSubId: subscribeId,
        ...
        await tagsPrint.print(allArticles)
    }
</script>

<ArticleMenu bind:this={articleMenu} {modeAdmin} />

<TagsPrint bind:this={tagsPrint} tag={$troc.tag} currency={$troc.currency} />
