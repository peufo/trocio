<script lang="ts">
  ...
  import { grey } from "$lib/material/utils/colors";
  ...
  function getScatterTraces(): Partial<Plotly.ScatterData>[] {
    ...
    stats.events
      ...
      .forEach((event) => {
        ...
        recover.y.push(
          mode === "sums" ? cursorSumRecover : cursorNumberRecover
        );
  ...
  function getLayout(): Partial<Plotly.Layout> {
    ...
    if ($troc?.schedule?.length) {
      layout.annotations = [
        {
          ...
          x: new Date($troc.schedule[0].open).getTime(),
        },
      ];
      const close = $troc.schedule.at(-1)?.close;
      if (close) {
        layout.annotations.push({
          text: "Fermeture",
          arrowcolor: grey[$isDarkTheme ? "lighten-2" : "darken-4"],
          x: new Date(close).getTime(),
        });
      }
      ...
      layout.xaxis!.range = [
        timeOpen < timeNow ? timeOpen : timeNow,
        timeClose,
      ];
    }
    if (mode === "sums") {
...
<div class="d-flex flex-column" style="gap: 1em;">
  ...
  <div bind:this={containerPlotStock} class="plot"></div>
  <div bind:this={containerHisto} class="plot"></div>
</div>
