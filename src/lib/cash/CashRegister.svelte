    ...
    function createUserLoader() {
        ...
        return () => {
            if (!subscribeId) subscribrIdLoaded = ''
                ...
                api<{ subscribeId: string }, { name: string }>(
                    '/api/users/name',
                    {
                        params: { subscribeId },
                    }
            ...
            {#if subscribeId && balance && Math.abs(balance) > 0.01}
                ...
                >
                    <Button
                        ...
                        on:click={openPaymentDialog}
                    >
                        Règler {renderAmount(-balance, $troc.currency)} en faveur
                        du
            ...
            >
                    ...
                    <div slot="tabs">
                        {#each TABS as { label, icon }}
                            <Tab class="rounded">
                                ...
                                {/if}
                                <span
                                    style={$isMobile ? 'font-size: 0.8em;' : ''}
                                >
                                    {label}
                                </span>
                            </Tab>
                ...
                {#key tabIndex}
                    ...
                    >
                        {#if tabIndex === '0'}
                            <Provide
                                bind:template={templateComponent}
                                {subscribeId}
                            />
                        {:else if tabIndex === '1'}
                            <Recover
