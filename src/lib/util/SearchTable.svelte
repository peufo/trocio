<script>
  ...
  function openMenu(field) {
    ...
    if (field.typeMenu == 'search' || field.typeMenu == 'or_search') {
      ...
      //Focus
      setTimeout(
        () => document.getElementById(`searchUser${field.dataName}`).focus(),
        200
      )
    }
  }
  ...
  function selectOption(field, selectedOption) {
    if (field.typeMenu == 'sort') {
      //remove all sort
      fields
        .filter((f) => f.typeMenu == 'sort')
        .forEach((f) => (f.queryValue = ''))
    }
...
<div style="display: flex; justify-content: center; flex-wrap: wrap-reverse;">
  <div style="display: flex; flex-direction: column;">
    ...
    <Table class="clickable" style="min-width: 690px; overflow-x: visible;">
      <thead>
        <tr>
          {#each fields.filter((f) => showAllFields || f.checked) as field}
            <td
              class="headCell"
              on:click={() => openMenu(field)}
              style={`width: ${field.cellWidth}px;`}
            >
              ...
              {:else if field.typeMenu == 'user'}
                <Menu
                  bind:this={menus[field.dataName]}
                  on:click={(e) => e.stopPropagation()}
                  style="overflow: visible; min-width: 200px;"
                >
                  TODO:
                  <!--
      ...
      <tbody>
        {#await dataPromise}
          <RowsPromise
            cellsWidth={fields
              .filter((f) => showAllFields || f.checked)
              .map((f) => f.cellWidth)}
          />
        {:then}
          {#each items as item, index}
            <tr
              ...
              class={selectedIndex == index ? 'row-selected' : ''}
            >
              {#each fields.filter((f) => showAllFields || f.checked) as field}
    ...
    <div style="align-self: flex-end;" class="w3-margin-top">
      {#if items.length && !noMoreData}
        <Button on:click={getMoreData} variant="outlined" color="secondary">
          Plus de résultats
        </Button>
      {/if}
  ...
  {#if !showAllFields}
    <div
      class="w3-margin-left w3-margin-bottom"
      style="align-self: flex-end; transform: translate(0px, 26.67px);"
    >
      <Menu>
        <div slot="activator">
        ...
        <div style="margin: 1em;">
          {#each fields as { label, checked, disabled }}
            <Checkbox bind:checked bind:disabled>
              {label}
            </Checkbox>
          {/each}
        </div>
