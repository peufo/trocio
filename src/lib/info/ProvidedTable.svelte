<script>
  ...
  function clickDeleteArticle(artId) {
    if (articleWaitValidationForDelete != artId)
      return (articleWaitValidationForDelete = artId)
        ...
        <Menu bind:this={statutFilterMenu}>
          <List>
            ...
            {#each STATUTS as statut, i}
              <ListItem on:click={() => (statutFilter = i)}
                ><span>{statut}</span></ListItem
              >
            {/each}
          </List>
        </Menu>
    ...
    {#if provided.length}
      ...
      {#each provided.slice(0, limitList) as article, i (article._id)}
        <tr transition:slide|local>
          ...
          <td>
            ...
            >
              {#await deleteArticlePromise}
                <i class="fas fa-recycle w3-spin" />
              {:then}
          ...
          <!-- Frais -->
          <td
            class:w3-opacity={!article.valided}
            class="fee"
            class:unvalided={!article.valided}
            on:click={() => dispatch('openTarifDialog')}
          >
            {article.fee.toFixed(2)}
            {@html article.sold
              ? ` <span class="w3-tiny">+</span> ${article.margin.toFixed(2)}`
              : ''}
          </td>
