<script>
  ...
  async function getArticles() {
    try {
      let res = await fetch(
        `/api/articles?troc=${troc}&search_name=${search}&skip=${articles.length}&limit=${LIMIT}`
      )
...
<div id="articles">
  ...
  <div style="max-width: 350px; margin: auto;">
    <TextField bind:value={search} clearable placeholder="Recherche">
      <div slot="prepend">
  ...
  {:then}
    <div in:fade class="flex" style="width: calc(100% + 5px);">
      {#each articles as article}
        <div
          class="simple-card w3-padding w3-display-container"
          style="margin: 5px 5px 0px 0px;"
        >
          {article.name}
          <br />
          <b class="w3-tiny w3-right" style="line-height: 1;"
            >{!isNaN(article.price) && Number(article.price).toFixed(2)}</b
          >
        </div>
      {/each}
    </div>
