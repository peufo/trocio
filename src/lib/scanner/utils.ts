...
export async function getMismatchRaison(
  ...
  // Détermine la raison du mismatch
  const [_article] = await api<Article[]>('/api/articles', {
    params: { exact__id: articleId },
  })
  if (!_article) return 'Ne correspond pas à un article Trocio'
  ...
  const providerSubId = _article.providerSubId
