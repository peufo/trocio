<script>
  ...
  async function loadTrocs(skip = 0) {
    let query = `/api/trocs/search?search=${search}&skip=${skip}`
  ...
  <div class="w3-col m6 w3-padding">
    ...
    {#if timeFilter}
      <div transition:fade|local class="container-filter">
        <TextField bind:value={start} on:change={newSearch} type="date">
          A partir du
        </TextField>

        <TextField bind:value={end} on:input={newSearch} type="date">
          Jusqu'au
        </TextField>
      </div>
    {/if}
...
<div class="results">
  {#each trocs
    .sort((a, b) => b.up - a.up)
    .slice(0, limitTrocsDisplay) as troc (troc._id)}
    <div
  ...
  {:else}
    <br />
    <span class="w3-large"> Aucun troc ne correspond à la recherche </span>
  {/each}
</div>
