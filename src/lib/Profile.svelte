<script>
    ...
    async function patchName() {
        try {
            let res = await fetch('/api/users/me', getHeader({name: userName}, 'PATCH'))
    ...
    async function patchMail() {
        try {
            let res = await fetch('/api/users/me', getHeader({mail: userMail}, 'PATCH'))
            let json = await res.json()
            if (!json.success) throw json.message
    ...
    async function validChangePassword() {
        try {
            let res = await fetch('/api/users/changepwd', getHeader({oldPassword, newPassword}))
            let json = await res.json()
            if (!json.success) throw json.message
