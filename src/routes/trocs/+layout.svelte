<script lang="ts">
  ...
  afterNavigate(() => {
    ...
    if (page.route.id === "/trocs/create") return (navigationActive = false);
    if (!$isMobile) return (navigationActive = true);
    if (page.route.id === "/trocs") return (navigationActive = true);
  });
</script>
...
>
  ...
  {#if $isMobile && !$isKeyboardOpen && !page.route.id?.startsWith("/trocs/[trocId]")}
