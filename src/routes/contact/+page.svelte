<script lang="ts">
  import { TextField, Textarea, Button } from "$lib/material";
  import { createMutation } from "@tanstack/svelte-query";

  import type { IMessageCreate } from "$lib/types";
  ...
  const querySend = createMutation({
    mutationFn: (data: IMessageCreate) =>
...
<main class="pa-4">
  ...
  {#if !$querySend.isSuccess}
    <div>
      <Textarea outlined autogrow bind:value={content}>
