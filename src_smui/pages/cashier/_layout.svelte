<script>
  ...
  onMount(async () => {
    if ($params.client === 'undefined') {
      ...
      try {
        let res = await fetch(`/api/users/name/${$params.client}`)
  ...
  function shortcut(e) {
    if (e.ctrlKey && e.key === 'Backspace') {
      ...
      else if (clientAnonym)
        notify.info('Le solde des clients anonymes est nul.')
  ...
  async function validPayment() {
    ...
    try {
      let res = await fetch(`/api/payments`, getHeader(payment))
      ...
      if (res.ok && json.success) {
        ...
        notify.success({
          ...
          text: `${newPayment.amount.toFixed(2)} ${newPayment.message}`,
        })
        return
      } else {
...
<main class:tipsOpen>
  ...
  {:else}
      ...
      <div id="userHandler">
        ...
        {#if clientOk && $details && $details.balance != 0}
          <div
            ...
            style="display: inline-block; transform: translate(0px, 5px);"
            class="w3-right"
          >
            <Button
              variant="raised"
          ...
          <div class="icon iconUser">
            {#if !clientAnonym}
              <i
                class:far={!clientOk}
                class:fas={clientOk}
                class="fa-user w3-large"
              />
            {:else}
              <i class="fas fa-user-secret w3-large" />
            {/if}
          </div>
        ...
        {#if !clientOk}
          <div in:fade={{ duration: 200 }} style="display: inline-block">
            ...
            >
              <i class="fas fa-user-plus w3-large" />&nbsp;Nouveau
            </Button>
            ...
            <Button
              ...
              class="w3-margin-left"
            >
              <i class="fas fa-user-secret w3-large" />&nbsp;Anonyme
            </Button>
          </div>
        {/if}
      </div>
    ...
    <div
      class:w3-padding={offsetWidth > 740}
      style="max-width: 1100px; margin: auto;"
    >
      {#if clientOk}
        <slot />
      {:else}
        <i class="fas fa-cash-register noUserLogo" />

        <!-- Cash register options-->
        <div style="position: fixed; bottom: 3em;">
          <FormField class="w3-large">
            <Switch bind:checked={$cashierOptions.autoPrintTag} />
            <span slot="label">
      ...
      {#if !!$details}
        <Content>
          <div class="w3-large" style="min-width: 400px;">
            ...
            {:else}
              {$details.balance > 0
                ? `Vous avez versé ${$details.balance.toFixed(2)} à ${
                    client.name
                  }`
                : `${client.name} vous a versé ${(-$details.balance).toFixed(
                    2
                  )}`}
            {/if}
          </div>
        ...
        <Actions>
          ...
          {:then}
            <Button
              on:click={() => (validPaymentPromise = validPayment())}
              variant="raised"
            >
  ...
  {#if !adminIntegration}
    <!--<Tips bind:open={tipsOpen} headerHeight={scoped.headerHeight} isCashier/>-->
