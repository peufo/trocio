<script>
    ...
    async function changePrefix(){
        try {
            let res = await fetch(`/api/trocs/${$troc._id}/trader/prefix`, getHeader({trader: $troc.trader[selectedTrader].user._id, prefix: selectedTraderPrefix}))
	...
	function addCashier(e) {
		fetch(`/api/trocs/${$troc._id}/cashier`, getHeader({cashier: e.detail._id}))
		.then(res => res.json())
        .then(updateTroc)
        .catch(console.trace)
    }
	...
	function addTrader(e) {
		fetch(`/api/trocs/${$troc._id}/trader`, getHeader({trader: e.detail._id, prefix: findNewPrefix()}))
		.then(res => res.json())
        .then(updateTroc)
        .catch(console.trace)
	}
	...
	function removeCashier(userId) {
		fetch(`/api/trocs/${$troc._id}/cashier/remove`, getHeader({cashier: userId}))
		.then(res => res.json())
        .then(updateTroc)
        .catch(console.trace)
    }
