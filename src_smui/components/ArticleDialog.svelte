<script>
    ...
    Object.keys({ ...article, ...articleEdited }).forEach((key) => {
      if (JSON.stringify(article[key]) !== JSON.stringify(articleEdited[key])) {
        edit.push({
          ...
          value:
            articleEdited[key] && !!articleEdited[key]._id
              ? articleEdited[key]._id
              : articleEdited[key],
        })
      }
    })
  ...
  async function editPrice(e) {
    ...
    if (!tarif) {
      tarifPromise = getTarif($troc._id, articleEdited.provider._id).then(
        updateFeeAndMargin
      )
    } else {
      updateFeeAndMargin()
  ...
  async function valid() {
    ...
    try {
      let res = await fetch(
        '/api/articles/edit',
        getHeader({ edits, article: article._id })
      )
      let json = await res.json()
...
<Dialog bind:this={editClientDialog} style="z-index: 8;">
  ...
  <Content style="height: 400px; width: 300px; overflow-y: visible;">
    <SearchUser
      id="editArticle"
      placholder="Définir le client"
      on:select={selectClient}
    />
    <br />
    <div class="w3-center">
      <Button
        on:click={selectClientAnonyme}
        variant="outlined"
        color="secondary"
      >
        <i class="fas fa-user-secret w3-large" />&nbsp;Client anonyme
      </Button>
    </div>
  </Content>
</Dialog>
...
<Dialog
  bind:this={dialog}
  on:MDCDialog:opening={onOpen}
  autoStackButtons="false"
  getEscapeKeyAction="false"
>
  <Title>
  ...
  <Content style="min-width: 500px;">
    {#if article.newPriceRequest}
      <span class="w3-text-red">
        Une demande de changement de prix ({article.newPriceRequest.price.toFixed(
          2
        )}) est en cours...
      </span>
    {/if}
    ...
    >
      <Item>
        ...
        <Text>
          <PrimaryText
            >Par {article.provider && article.provider.name}</PrimaryText
          >
          <SecondaryText
            >Le {new Date(article.createdAt).toLocaleString()}</SecondaryText
          >
        </Text>
      </Item>
      ...
      {#if articleEdited.valided || articleEdited.refused}
        <Item>
          <Graphic
            class={articleEdited.valided ? 'w3-text-blue' : 'w3-text-red'}
          >
          ...
          <Text>
            ...
            <SecondaryText
              >Le {new Date(
                articleEdited.valided || articleEdited.refused
              ).toLocaleString()}</SecondaryText
            >
          </Text>

          {#if !articleEdited.sold && !articleEdited.recover}
            <Meta
              on:click={removeValidation}
              title={articleEdited.valided
                ? 'Annuler la validation'
                : 'Annuler le refus'}
            >
              <i class="far fa-trash-alt w3-large button-icon" />
            </Meta>
          {/if}
        </Item>
      {/if}
        ...
        <Item>
          {#if articleEdited.sold}
            ...
            <Text>
              <PrimaryText>
                ...
                <span
                  on:click={() => editClientDialog.open()}
                  class="clickable"
                  title="Définir le client"
                >
                  à {@html (articleEdited.buyer && articleEdited.buyer.name) ||
                    'un client anonyme <i class="fas fa-user-secret w3-opacity"></i>'}
                </span>
              </PrimaryText>
              <SecondaryText
                >Le {new Date(
          ...
          {:else}
            ...
            <Button
              on:click={recover}
              color="secondary"
              title="Rendre l'article à son propriétaire"
            >
              <Label><i class="fas fa-undo" /> Rendre</Label>
            </Button>
            <Button on:click={sold} title="Vendre l'article à un client">
              <Label><i class="fas fa-shopping-cart" /> Vendre</Label>
            </Button>
          {/if}
        </Item>
      {:else if !articleEdited.refused}
        <Item>
          ...
          <Button
            on:click={refuseValidation}
            color="secondary"
            title="Refusé l'article proposé"
          >
            <Label><i class="fa fa-times" /> Refuser</Label>
          </Button>
          <Button
            on:click={acceptValidation}
            title="Accepter l'article proposé"
          >
            <Label><i class="fa fa-check" /> Valider</Label>
          </Button>
        </Item>
      {/if}
    </List>
    ...
    <div>
      ...
      {#await tarifPromise}
        <i class="fas fa-spinner w3-spin" />
        <span>...tarif</span>
      {:then}
        <span>Frais: {articleEdited.fee && articleEdited.fee.toFixed(2)}</span
        ><br />
        <span
          >Marge: {articleEdited.margin &&
            articleEdited.margin.toFixed(2)}</span
        ><br />
      {/await}
    </div>
  </Content>
    ...
    <div transition:slide>
      <Actions>
        <Button color="secondary">
          <Label><i class="fa fa-times" /> Annuler</Label>
        </Button>
        <Button on:click={valid}>
          <Label><i class="fa fa-check" /> Sauvegarder</Label>
        </Button>
      </Actions>
    </div>
  {/if}
