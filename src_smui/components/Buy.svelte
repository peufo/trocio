<script>
  ...
  async function getArticles() {
    ...
    req += `&select_statut=valided`
    req += `&or_search_name=${search}`
    req += `&or_search_ref=${search}`
    req += `${
      $params.client && $params.client !== 'undefined'
        ? `&providernot=${$params.client}`
        : ''
    }`
    ...
    try {
      ...
      if (res.ok) {
        ...
        articles = articles.filter(
          (a) => cart.map((c) => c._id).indexOf(a._id) == -1
        )
  ...
  async function validBuy() {
    ...
    try {
      let res = await fetch(
        '/api/articles',
        getHeader(patchedArticles, 'PATCH')
      )
      let json = await res.json()
      if (res.ok && json.success) {
        let newPurchases = json.message
        $details.purchases = [...newPurchases, ...$details.purchases]
        $details.purchases[0].soldTime = new Date(
          $details.purchases[0].sold
        ).getTime()
        let newBuySum = newPurchases
          .map((art) => art.price)
    ...
    {:then}
      {#each articles as article, index (article._id)}
        <div
          ...
          in:receive|local={{ key: article._id }}
          out:send|local={{ key: article._id }}
        ...
        >
          <Article
            ...
            clickable
            on:select={() => buy(index)}
            comment={`Fourni par ${article.provider.name}`}
          />
        </div>
      {/each}
      ...
      {:else if articles.length}
        {#if !noMoreResults}
          ...
          {:then}
            ...
            <div
              class="w3-col underline-div w3-center w3-opacity"
              on:click={getMoreResults}
            >
              <span class="underline-span"
                >Plus de résultats ({articles.length} / {countArticleAvailable})</span
              >
            </div>
          {/await}
        {/if}
  ...
  <div class="w3-col m6">
    <div class="w3-margin-left w3-border w3-round w3-padding">
      ...
      {:then}
        {#if cart.length}
          <Button
            on:click={() => (buyPromise = validBuy())}
            class="w3-right"
            variant="raised"
          >
            Valider l'achat de{cart.length <= 1
              ? ` l'article`
              : `s ${cart.length} articles`} : {cart
              .map((a) => a.price)
              .reduce((acc, cur) => (acc += cur))
              .toFixed(2)}
          </Button>
      ...
      {#each cart as article, index (article._id)}
        <div
          ...
          animate:flip={{ duration: 200 }}
        >
          <Article {article} on:remove={() => removeArticle(index)} />
        </div>
      {:else}
    ...
    <div class="w3-margin-left">
      ...
      {#each $details.purchases
        ...
        <div
          ...
          animate:flip={{ duration: 200 }}
        >
          <Article {article} timeKey={'soldTime'} />
        </div>
      {:else}
      ...
      {#if $details.purchases.length > LIMIT_LIST_B}
        <div
          on:click={() => (LIMIT_LIST_B += 25)}
          class="underline-div w3-center"
        >
          <span class="underline-span w3-opacity">
            Afficher plus d'éléments ({$details.purchases.length -
              LIMIT_LIST_B})
          </span>
        </div>
      {/if}
