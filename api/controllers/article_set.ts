    ...
    try {
        ...
        const articlesCreated = await Article.insertMany(
            articles.map((art, index) => {
                ...
                if (!article.ref)
                    article.ref = String(newRef + nbAttributedRef++)
    ...
    try {
        ...
        if (accessor.role !== 'admin' && accessor.role !== 'cashier') {
            if (String(article.providerId) !== req.session.user._id)
                throw 'Not allowed'
            if (article.valided)
                throw `Valided article can't be edited by provider`
        }
...
export const validArticles: RequestHandler = async (req, res, next) => {
    try {
        ...
        if (
            articles.filter(
                (art) =>
                    !art.valided && !art.refused && !art.sold && !art.recover
...
async function ensureCanEdit(req: Request, needFromSameProvider = true) {
    try {
        ...
        if (!articleId && !articlesId)
            throw 'articleId or articlesId is required'
        ...
        if (isArray) {
            ...
            if (
                articles.map((a) => a.trocId?.valueOf()).filter(beUnique)
                    .length > 1
            )
            ...
            if (
                ...
                articles.map((a) => a.providerSubId.valueOf()).filter(beUnique)
                    .length > 1
            )
